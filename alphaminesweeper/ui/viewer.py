...
class Viewer(QWidget):
    def __init__(self,status_boards, positions, is_win, sizeunit=50):
        ...
        self.index = 0
        self.boardinfo = self.status_boards[self.index]
    ...
    def keyPressEvent(self,e):
        if e.key() == Qt.Key_Up:
            if self.index + 1 < len(self.status_boards):
                self.index += 1
                self.boardinfo = self.status_boards[self.index]

                if self.index == len(self.status_boards) - 1:
                    self.isgameend = True
    ...
    def drawmines(self, qp):
        ...
        if self.index > 0:
            ...
            i,j = self.positions[self.index-1]
            qp.drawRect(i*self.sizeunit + x, j*self.sizeunit + y, dx, dy)
